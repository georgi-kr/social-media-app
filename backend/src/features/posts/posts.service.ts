...
export class PostsService {
  ...
  public async allPostsByFollowed(
    ...
    try {
      posts = await this.postsRepository.find({
        where: {
          id: In([...follwedUsersPostsIds, ...currentUserPostsIds]),
          isDeleted: false,
        },
        order: { postedOn: 'DESC' },
        take: +take,
  ...
  public async findPostByDescription(
    description: string,
    ...
    const filteredPosts = posts.filter((post: Post) => {
      if (post.isPrivate && user.id !== post.user.id) {
        return followed.some(follow => follow.id === post.user.id);
      } else {
        return true;
      }
    });
